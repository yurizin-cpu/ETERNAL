...
/* ETERNAL — audio controller v3 */
(() => {
  const audio = document.getElementById('bgMusic');
  const btn = document.getElementById('musicToggle');
  if (!audio || !btn) return;

  let enabled = localStorage.getItem('eternal_music_enabled');
  enabled = enabled === null ? true : enabled === 'true';

  const savedVolume = Number(localStorage.getItem('eternal_music_volume'));
  audio.volume = Number.isFinite(savedVolume) ? Math.max(0, Math.min(1, savedVolume)) : 0.7;

  function render() {
    btn.textContent = enabled ? '🔊' : '🔇';
    btn.setAttribute('aria-label', enabled ? 'Desativar música' : 'Ativar música');
    btn.title = enabled ? 'Desativar música' : 'Ativar música';
    btn.disabled = false;
  ...
  btn.addEventListener('click', async (e) => {
    e.stopPropagation();
    enabled = !enabled;
    localStorage.setItem('eternal_music_enabled', String(enabled));
    render();
    if (enabled) {
      await playMusic();
    } else {
  ...
  // A user gesture anywhere can unlock autoplay on mobile, but never changes
  // the enabled/disabled state.
  document.addEventListener('pointerdown', () => {
    if (enabled && audio.paused) playMusic();
  }, { passive: true });
  ...
  audio.addEventListener('volumechange', () => {
    localStorage.setItem('eternal_music_volume', String(audio.volume));
  });

  audio.addEventListener('error', () => {
    console.warn('ETERNAL: não foi possível carregar ./music.mp3', audio.error);
  });

  render();
  // Try immediately on desktop; mobile may reject until gesture.
  playMusic();
})();
